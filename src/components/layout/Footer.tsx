...
export function Footer({ footerNavigation, socialLinks, contactEmail }: FooterProps) {
  const navLinks = footerNavigation?.length ? footerNavigation : NAV_ITEMS
      ...
      <Container className="py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div>
                  ...
                  >
                    {SOCIAL_ICONS[social.platform.toLowerCase()] ? (
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                        <path d={SOCIAL_ICONS[social.platform.toLowerCase()]} />
                      </svg>
                    ) : (
            ...
            {email && <p className="text-white/60 text-sm">{email}</p>}
          </div>
        </div>
